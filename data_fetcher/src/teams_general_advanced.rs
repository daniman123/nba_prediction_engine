...
pub async fn fetch_teams_general_advanced() -> Result<(), FetchError> {
    // API Endpoint
    let endpoint = Endpoint::TeamsGeneralAdvanced.url();
    // File Name for future reference
    let file_name = Endpoint::TeamsGeneralAdvanced.file_name();
